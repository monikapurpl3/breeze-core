...
- **Rollout clamp:** `AC_MIN_AUTH_VERSION` (default `1`) accepts both and nudges v1 clients (`X-Breeze-Upgrade` header). A **new** device can still enrol as v1 at the default floor — required, because the web UI and the CLIs are v1-only — so v1 back-compat is *not* an escalation (enrolling v1 needs the same API key + LAN approval as v2, and a v2 device can never be downgraded to a bearer). Set the floor to `2` once your clients are updated: v1 is then refused **both** at control (`426`) **and** at enrollment (no new v1 credential is minted), so the server is effectively v2-only. Enrolled v1 devices upgrade in place via `POST /api/auth/upgrade` (no re-pairing). **Migrate the web UI + CLIs before clamping.**
